...
async function convertTiles(indicies_index, placeAir, size, keepStates) {
  ...
  var blockIdentifiers =
    tempstructure.value.structure.value.block_indices.value.value[
      indicies_index
    ].value;
  var palette =
    tempstructure.value.structure.value.palette.value.default.value
      .block_palette.value.value;

  var unsortedTiles = [];
  postMessage({
    type: "progress",
    message: "Indexing " + blockIdentifiers.length + " blocks...",
  });
  let indexprogresses = [];
  for (var i = 0; i < blockIdentifiers.length; i++) {
    ...
    if (
      Math.floor((i / blockIdentifiers.length) * 100) % 10 === 0 &&
      Math.floor((i / blockIdentifiers.length) * 100) !== 0 &&
      !indexprogresses.includes(Math.floor((i / blockIdentifiers.length) * 100))
    ) {
      postMessage({
        type: "progress",
        message:
          "Indexing " +
          Math.floor((i / blockIdentifiers.length) * 100) +
          "% complete",
      });
  ...
  postMessage({
    type: "progress",
    message: "Finished indexing blocks in this layer",
  });
  ...
  postMessage({
    type: "progress",
    message: "Sorting tiles into fill command shapes...",
  });
  while (sortedTiles.length > 1 /* && currentShape < 1000*/) {
    ...
    postMessage({
      type: "progress",
      message:
        "Found shape containing " + shapes[currentShape].length + " blocks",
    });
  }
  postMessage({
    type: "progress",
    message: "Finished identifying shapes in this layer",
  });
      ...
      if (tilePalette != -1) {
        ...
        if (whitelistStates(palette[tilePalette].states.value)) {
          ...
          if (placeAir || (!placeAir && id != "minecraft:air")) {
            commands.push(
              "fill " +
                getRelativeCoords(xyz1) +
                " " +
                getRelativeCoords(xyz2) +
                " " +
                id +
                (keepStates ? states : "")
            );
  ...
  postMessage({
    type: "progress",
    message: "Finished converting all shapes to commands for this layer.",
  });
...
async function structureToFunction(
  ...
  var structure_world_origin =
    tempstructure.value.structure_world_origin.value.value;

  //Entities
  var entities = tempstructure.value.structure.value.entities.value.value;
  if (
    tempstructure.value.structure.value.entities.value.type != "end" &&
    includeEntities
  ) {
    ...
    for (var i = 0; i < entities.length; i++) {
      //Entities
      var coords = getFunctionEntityCoords(
        structure_world_origin,
        entities[i].Pos.value.value
      );
      output.push(
        "summon " +
          entities[i].identifier.value +
          " " +
          getRelativeCoords(
            getFunctionEntityCoords(
              structure_world_origin,
              entities[i].Pos.value.value
            )
          ) +
          " none " +
          (entities[i].CustomName ? entities[i].CustomName.value : "")
      );
            ...
            if (entities[i].Armor.value.value[a].Name.value != "") {
              output.push(
                "replaceitem entity @e[type=" +
                  ...
                  armor[a] +
                  " 0 " +
                  entities[i].Armor.value.value[a].Name.value +
                  " " +
                  entities[i].Armor.value.value[a].Count.value +
                  " " +
                  entities[i].Armor.value.value[a].Damage.value
              );
            }
          }
        }
          ...
          if (entities[i].Mainhand.value.value[0].Name.value != "") {
            output.push(
              "replaceitem entity @e[type=" +
                ...
                ",z=~" +
                coords[2] +
                ",r=1,c=1] slot.weapon.mainhand 0 " +
                entities[i].Mainhand.value.value[0].Name.value +
                " " +
                entities[i].Mainhand.value.value[0].Count.value +
                " " +
                entities[i].Mainhand.value.value[0].Damage.value
            );
          }
        }
          ...
          if (entities[i].Offhand.value.value[0].Name.value != "") {
            output.push(
              "replaceitem entity @e[type=" +
                ...
                ",z=~" +
                coords[2] +
                ",r=1,c=1] slot.weapon.offhand 0 " +
                entities[i].Offhand.value.value[0].Name.value +
                " " +
                entities[i].Offhand.value.value[0].Count.value +
                " " +
                entities[i].Offhand.value.value[0].Damage.value
            );
          }
        }
            ...
            if (entities[i].ChestItems.value.value[a].Name.value != "") {
              output.push(
                "replaceitem entity @e[type=" +
                  ...
                  ",z=~" +
                  coords[2] +
                  ",r=1,c=1] slot.inventory " +
                  entities[i].ChestItems.value.value[a].Slot.value +
                  " " +
                  entities[i].ChestItems.value.value[a].Name.value +
                  " " +
                  entities[i].ChestItems.value.value[a].Count.value +
                  " " +
                  entities[i].ChestItems.value.value[a].Damage.value
              );
            }
          }
        }
      ...
      if (entityRotation) {
        output.push(
          "execute @e[type=" +
            ...
            coords[2] +
            ",r=1,c=1] ~~~ /tp @s ~ ~ ~ " +
            entities[i].Rotation.value.value.join(" ")
        );
      }
    ...
    postMessage({
      type: "progress",
      message: "Finished converting all entities",
    });
  }
  ...
  if (tileContainerItems) {
    ...
    for (var i = 0; i < tileentities.length; i++) {
      ...
      if (tile.Items) {
        ...
        for (var a = 0; a < tileitems.length; a++) {
          if (tileitems[a].Name.value != "") {
            output.push(
              "replaceitem block " +
                getRelativeCoords(tilecoords) +
                " slot.container " +
                (tileitems[a].hasOwnProperty("Slot")
                  ? tileitems[a].Slot.value
                  : a) +
                " " +
                tileitems[a].Name.value +
                " " +
                tileitems[a].Count.value +
                " " +
                tileitems[a].Damage.value
            );
          }
        }
      }
      postMessage({
        type: "progress",
        message:
          "Finished converting tile entity (" +
          (i + 1) +
          "/" +
          tileentities.length +
          ")",
    ...
    postMessage({
      type: "progress",
      message: "Finished converting tile entities",
    });
  }
  ...
  if (includeBlocks) {
    ...
    postMessage({
      type: "progress",
      message: "Finished converting main block layer",
    });
  }

  postMessage({
    type: "progress",
    message: "Conversion finished. Creating file...",
  });
